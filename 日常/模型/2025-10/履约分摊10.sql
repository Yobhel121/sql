drop table tmp.fulfillment_10;
create table tmp.fulfillment_10 as
select
    flag_names  订单标记
     ...
     , order_sku.share_favourable_after_fee_js2  分摊后金额
...
from (
    select * from(
        ...
        where is_delete = 0 and seller_memo not like '%测试单%' and gmt_create > date_format(date_sub('2025-11-13', 90), '%Y-%m-%d')
    )tmp where n =1
           ...
           and shop_id  not in('1723838592174621184','1726051235912319488','1695714490516406912','1739161925332304256','1799908914256478592','2240803665545987200','2083305436995192192','2069054003205801856','2164494330354370304'
                              ,'1695714467053470336','1729038499259942016','2005022954993549952','1768215165143156736','1777563627112203648','1850175248469395456')
           and warehouse_name not in('微瑕品仓','易鹏供应链分仓')
           and shop_name not like '%线下中心%'
           and flag_names not like '%平台发货%'
           and flag_names not like '%外部发货%'
           and pay_time like '2025-10%'
) j_order
left join(
    select
        a.trade_no, -- 订单编号
        ...
        -- 分摊后金额
        if(goods_field3 = '虚拟编码',0,case when b.sell_total is not null then cast(b.sell_total as decimal(10,2)) - cast(b.share_favourable_fee as decimal(10,2))  else a.share_favourable_after_fee end) share_favourable_after_fee_js,
        -- 分摊后金额
        case when b.sell_total is not null then cast(b.sell_total as decimal(10,2)) - cast(b.share_favourable_fee as decimal(10,2))  else a.share_favourable_after_fee end share_favourable_after_fee_js2,
        -- 组合装分摊后金额
        cast(a.share_favourable_after_fee as decimal(10,2)) p_share_favourable_fee_js
    from(select trade_no,gmt_create,pay_time from ods.jky_qm_trades_fullinfo where is_delete = 0 and seller_memo not like '%测试单%' and gmt_create > date_format(date_sub('2025-11-12', 90), '%Y-%m-%d') and pay_time like '2025-10%') t -- 销售单查询
    left join(select * from ods.jky_qm_trades_fullinfo_goodsdetail_2 where dt = '2025-11-12') a on t.trade_no = a.trade_no -- 销售单查询_货品详情
    left join(select * from ods.jky_qm_trades_fullinfo_assembly_goodsdto where dt = '2025-11-12') b on a.trade_no = b.trade_no and a.trade_id = b.trade_id and a.sub_trade_id = b.sub_trade_id -- 销售单查询_组合装子件列表
    left join(select * from ods.jky_storage_goodsinfo where dt = '2025-11-12' and is_delete = 0) c on nvl(b.goods_no,a.goods_no) = c.goods_no and nvl(b.barcode,a.barcode) = c.sku_barcode -- 分页查询货品信息
    left join(
        select product_code as goods_no,standard_cost adjustment_cost from ods.gyl_product where dt = '2025-11-12' -- 成本调整_template
    )d on c.goods_no = d.goods_no
)order_sku on j_order.trade_no = order_sku.trade_no
left join(select * from ods.jky_sales_channel where dt = '2025-11-12') channel on j_order.shop_id = channel.channel_id -- 销售渠道查询
left join(select * from dim.jky_storage_goodsinfo where dt = '2025-11-12' ) tmcate on order_sku.sku_id = tmcate.sku_id
left join(
    select order_id,storage_fee + handling_fee as num_fulfillment, operation_fee_total + material_fee_total + express_fee_total as weight_fulfillment  from ods.gyl_order_fulfillment where dt = '2025-11-12'
...
left join(
    select order_id,cate from(
        select *,row_number() over (partition by order_id order by type desc) n from (
            select distinct sub_order_id as order_id,order_cate cate,1 as type from dwd.trd_tb_unpaid_order where dt = '2025-11-12' -- 交易域-淘宝未结算订单数据
            union
            select distinct sub_order_id,order_cate,2 as type from dwd.trd_tb_living where dt = '2025-11-12'    -- 订单域-淘宝直播订单
        )tmp
    )t where n =1
    union
    select distinct order_id,business_cate from dwd.trd_dy_alliance_order where dt = '2025-11-12'  -- 抖音-联盟订单明细
    union
...
left join(select * from ods.gyl_product_material_cost where dt = '2025-11-12') product_material on order_sku.trade_no = product_material.order_id and order_sku.goods_no = product_material.product_code and order_sku.sku_cnt = product_material.product_count
;




show create table   ods.jky_qm_trades_fullinfo_goodsdetail_2;

select * from ods.jky_qm_trades_fullinfo_goodsdetail_2 limit 10;
