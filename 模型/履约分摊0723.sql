# drop table tmp.fulfillment_0723;
# create table tmp.fulfillment_0723 as
select
#     share_favourable_after_fee_js,
#     sum(share_favourable_after_fee_js) over(partition by channel_code,date_format(pay_time, '%Y-%m-%d')),
#     sum(if(trade_type not in(7,8,12) and payment = 0 ,gift_cost * sku_cnt,0)) over(partition by channel_code,date_format(pay_time, '%Y-%m-%d')),
#     share_favourable_after_fee_js/ sum(if(share_favourable_after_fee_js>0,share_favourable_after_fee_js,0)) over(partition by channel_code,date_format(pay_time, '%Y-%m-%d')) * sum(if(trade_type not in(7,8,12) and payment = 0 ,gift_cost * sku_cnt,0)) over(partition by channel_code,date_format(pay_time, '%Y-%m-%d')),
#     if(trade_type in(7,8,12),0,share_favourable_after_fee_js/ sum(if(share_favourable_after_fee_js>0,share_favourable_after_fee_js,0)) over(partition by channel_code,date_format(pay_time, '%Y-%m-%d')) * if(j_order.payment = 0, sum(if(trade_type in(7,8,12),0,gift_cost * sku_cnt)) over(partition by channel_code,date_format(pay_time, '%Y-%m-%d')),0)),
#     payment,
#     trade_type,
    flag_names  订单标记
...
from (
    select * from(
        ...
        where is_delete = 0 and seller_memo not like '%测试单%' and gmt_create > date_format(date_sub('2025-08-07', 90), '%Y-%m-%d')
    )tmp where n =1
...
left join(
    ...
    from(select trade_no,gmt_create,pay_time from ods.jky_qm_trades_fullinfo where is_delete = 0 and seller_memo not like '%测试单%' and gmt_create > date_format(date_sub('2025-08-07', 90), '%Y-%m-%d') and pay_time like '2025-07%') t -- 销售单查询
    left join(select * from ods.jky_qm_trades_fullinfo_goodsdetail where dt = '2025-08-07') a on t.trade_no = a.trade_no -- 销售单查询_货品详情
    left join(select * from ods.jky_qm_trades_fullinfo_assembly_goodsdto where dt = '2025-08-07') b on a.trade_no = b.trade_no and a.trade_id = b.trade_id and a.sub_trade_id = b.sub_trade_id -- 销售单查询_组合装子件列表
    left join(select * from ods.jky_storage_goodsinfo where dt = '2025-08-07' and is_delete = 0) c on nvl(b.goods_no,a.goods_no) = c.goods_no and nvl(b.barcode,a.barcode) = c.sku_barcode -- 分页查询货品信息
    left join(
        select product_code as goods_no,standard_cost adjustment_cost from ods.gyl_product where dt = '2025-08-07' -- 成本调整_template
    )d on c.goods_no = d.goods_no
)order_sku on j_order.trade_no = order_sku.trade_no
left join(select * from ods.jky_sales_channel where dt = '2025-08-07') channel on j_order.shop_id = channel.channel_id -- 销售渠道查询
left join(select * from dim.jky_storage_goodsinfo where dt = '2025-08-07' ) tmcate on order_sku.sku_id = tmcate.sku_id
left join(
    select order_id,storage_fee + handling_fee as num_fulfillment, operation_fee_total + material_fee_total + express_fee_total as weight_fulfillment  from ods.gyl_order_fulfillment where dt = '2025-08-07'
...
left join(
    select order_id,cate from(
        select *,row_number() over (partition by order_id order by type desc) n from (
            select distinct sub_order_id as order_id,order_cate cate,1 as type from dwd.trd_tb_unpaid_order where dt = '2025-08-07' -- 交易域-淘宝未结算订单数据
            union
            select distinct sub_order_id,order_cate,2 as type from dwd.trd_tb_living where dt = '2025-08-07'    -- 订单域-淘宝直播订单
        )tmp
    )t where n =1
    union
    select distinct order_id,business_cate from dwd.trd_dy_alliance_order where dt = '2025-08-07'  -- 抖音-联盟订单明细
    union
...
left join(select * from ods.gyl_product_material_cost where dt = '2025-08-07') product_material on order_sku.goods_no = product_material.product_code
where channel_name = '盛弘-京东-秋田满满辅食官方旗舰店-代运营' and pay_time like '2025-07-21%'
;
