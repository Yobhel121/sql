...
CREATE TABLE ods.rpa_rdy_order_info (
    id                              bigint AUTO_INCREMENT COMMENT '主键ID，自增',
    ...
    dt                                date comment '分区日期',
    PRIMARY KEY (id, dt)
) PARTITION BY VALUE(dt) COMMENT='热度云-订单-视频号-订单查询';


CREATE TABLE ods.rpa_htzt_finance (
    id                         bigint AUTO_INCREMENT COMMENT '主键ID，自增',
    shop_id                    varchar COMMENT '店铺ID',
    ...
    dt                         date comment '分区日期',
    PRIMARY KEY (id, dt)
) PARTITION BY VALUE(dt) COMMENT='海豚智投-短视频-财务报表';

CREATE TABLE ods.`rpa_tb_sycm_product_performance` (
    ...
    dt                        date comment '分区日期',
    PRIMARY KEY (id, dt)
) PARTITION BY VALUE(dt) COMMENT='淘宝-生意参谋-业务专区-聚划算数据中心-商品明细';

CREATE TABLE ods.`rpa_tb_sycm_product_sales_stats` (
  `id` bigint AUTO_INCREMENT COMMENT '主键ID',
  ...
  `updated_at` timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
   dt                        date comment '分区日期',
   PRIMARY KEY (id, dt)
) PARTITION BY VALUE(dt) COMMENT='淘宝-生意参谋-品类-百亿补贴(新)-商品汇总';
